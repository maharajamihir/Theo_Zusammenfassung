...
- jedes Blatt mit einem Zeichen aus $\sum \cup \{\epsilon\}$ beschriftet ist
- jeder innere Knoten mit einem $A\in V$ beschriftet ist, falls Nachfolger: als $X_1, ..., X_n \in V\cup \sum \cup \{\epsilon\}$ beschriftet. Dann ist $A \rightarrow X_1, ..., X_n$ eine Produktion in $P$
...
*** Beispiel 4.24 Epsilonproduktion
...
1. Füge für jedes $a \in \Sigma$ das in einer rechten Seite der Länge $\geq 2$ vorkommt ein $X_a$ ein und füge $X_a \rightarrow a$ hinzu
